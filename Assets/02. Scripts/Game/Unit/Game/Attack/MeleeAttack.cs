...
public class MeleeAttack : IAttack
{
    ...
    public MeleeAttack(BaseUnit unit)
    {
        m_unit = unit;
    }
    ...
    public void Action(BaseUnit unit, float delay = 0.5f)
    {
        m_unit.StartCoroutine(Co_Damage(unit, delay));
    }

    private IEnumerator Co_Damage(BaseUnit unit, float delay)
    {
        float elapsed_time = 0f;
        float target_time = delay;
        ...
        if (unit.Health.IsDead || !m_is_attack)
        {
            yield break;
        }

        unit.Health.UpdateHP(-m_current_atk);
        m_unit.CreateDamageIndicator(unit.transform.position);
    }
}
